...
- A web build, so ColumnKit works in a browser as well as on the desktop. Same source, bundled into one file because the web extension host has no module loader. The daily update check is skipped there, since a browser install comes from the Marketplace and keeps itself current.
